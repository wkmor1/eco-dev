...
RUN    RVER=$(curl https://cran.r-project.org/banner.shtml | grep src/base | egrep -o '[0-9]+(\.[0-9]+)+' | head -1) \
    ...
    && curl \
         ...
         -o rstudio.deb https://download2.rstudio.org/server/trusty/amd64/rstudio-server-1.2.5019-amd64.deb \
...
RUN    pip3 install --upgrade pip \
    && /usr/local/bin/pip3 install --upgrade nvidia-ml-py3 tensorflow-gpu tensorflow-probability --ignore-installed six
